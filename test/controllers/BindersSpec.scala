...
import models.ObligationsQueryParams
...
class BindersSpec extends UnitSpec with GuiceOneAppPerSuite {
  ...
  "Obligations query binder" when {

    "it receives a valid obligation query" should {

      "bind to an `ObligationsQueryParams` instance" in {
        import play.api.mvc.QueryStringBindable.bindableString

        val result = Binders.obligationsQueryParamsBinder.bind("", Map(
          "from" -> Seq("2017-01-01"),
          "to" -> Seq("2017-03-31"),
          "status" -> Seq("O")
        ))

        result shouldBe Some(Right(ObligationsQueryParams(
      ...
      "unbind the ObligationsQueryParams to a string for its keys" in {
        import play.api.mvc.QueryStringBindable.bindableString

        val result = Binders.obligationsQueryParamsBinder.unbind("from", ObligationsQueryParams(
          LocalDate.parse("2017-01-01"),
          LocalDate.parse("2017-03-31"),
          "O"
        ))
    ...
    "it receives an invalid obligation query" should {
